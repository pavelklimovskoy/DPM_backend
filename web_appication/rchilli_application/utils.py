...
from application_core.settings import RCHILLI_API_KEY, RCHILLI_USER
import requests
...
class RchilliConnector:
    ...
    def __init__(self):
        if not RchilliConnector.__instance:
            ...
            self.USER_NAME = RCHILLI_USER

    def payload_for_request(self, keyword: str):
        return json.dumps(
            {
    ...
    def skill_search(self, skill_name):
        payload = self.payload_for_request(skill_name)
    ...
    def skill_autocomplete(self, skill_name):
        ...
        try:
            ...
            resp = response["SkillAutoComplete"]
            return {"options": resp}
        except Exception as e:
            print(e)

    def job_search(self, job_name):
        payload = self.payload_for_request(job_name)
